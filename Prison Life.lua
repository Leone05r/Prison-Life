        ...
        for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and (p.TeamColor.Name == "Really red" or (p.Team and p.Team.Name == "Criminals")) then
                ...
                local act = CreateBtn("ARREST", 0, row, function(b)
                    ...
                    task.spawn(function()
                        for i = Config.CooldownTime, 1, -1 do Config.CurrentCD = i task.wait(1) end
                        Config.GlobalCooldownActive = false
                    end)
                end, Color3.fromRGB(0, 120, 255))
                if Config.GlobalCooldownActive then act.Text = Config.CurrentCD .. "s" act.BackgroundColor3 = Color3.fromRGB(80, 80, 80) end
